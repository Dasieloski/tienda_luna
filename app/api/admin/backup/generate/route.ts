...
// JSON.stringify no soporta BigInt nativamente; Prisma devuelve BigInt en algunos campos
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const zip = new JSZip();
    zip.file("metadata.json", JSON.stringify(metadata, jsonReplacer, 2));
    zip.file("data.json", JSON.stringify(data, jsonReplacer, 2));
